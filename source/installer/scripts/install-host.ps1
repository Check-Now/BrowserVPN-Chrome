...
$hostPath = Join-Path $InstallDir "native-host\browservpn-host.exe"
...
$manifestPath = Join-Path $InstallDir "native-host\com.browservpn.host.json"
...
if ($SingBoxPath) {
  [Environment]::SetEnvironmentVariable("BROWSERVPN_SING_BOX", $SingBoxPath, "User")
}
