...
4. Build `native-host/dist/browservpn-chrome-host.exe`.
5. Install the host with `installer/scripts/install-host.ps1`.
6. Put `sing-box.exe` under `%LOCALAPPDATA%\BrowserVPN-Chrome\bin\sing-box.exe` or set `BROWSERVPN_CHROME_SING_BOX`.
